...
import { autoLinkDiscoveryAction, setDiscoveryPageIdAction } from "../actions";

export function DiscoveryLinkCell({
  clientId,
  clientName,
  initialPageId,
}: {
  ...
  function handleAutoLink() {
    ...
    startTransition(async () => {
      const res = await autoLinkDiscoveryAction(clientId, clientName);
      if (res.ok) {
  ...
  function handleManualSave() {
    ...
    setError(null);
    startTransition(async () => {
      const res = await setDiscoveryPageIdAction(clientId, manualInput.trim());
      if (res.ok) {
        setPageId(res.pageId);
        setShowManual(false);
        setManualInput("");
